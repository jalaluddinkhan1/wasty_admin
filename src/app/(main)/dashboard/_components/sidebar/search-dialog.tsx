...
function buildSearchItems(navGroups: readonly NavGroup[]): SearchItem[] {
  const groupLabels = new Set(navGroups.flatMap((group) => (group.label ? [group.label] : [])));
...
export function SearchDialog({ navGroups }: SearchDialogProps) {
  const [open, setOpen] = React.useState(false);
  const [query, setQuery] = React.useState("");
  const router = useRouter();

  const searchItems = React.useMemo(() => buildSearchItems(navGroups), [navGroups]);
